<?php
    ...
    $adicionar_usuario = new Dados("nome do banco de dados", "host", "nome de usuario", "senha");

    // Arrays para armazenar mensagens de erro e sucesso.
    $mensagem_erro = [];
    $mensagem_sucesso = [];

    // Se o botão 'adicionar-novo-usuario' for clicado, tudo abaixo será executado.
    if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['adicionar-novo-usuario'])){
        ...
        $_SESSION['senha'] = addslashes(strip_tags(trim($_POST['senha'])));
        $_SESSION['repita-senha'] = addslashes(strip_tags(trim($_POST['repita-senha'])));
        ...
        if(strlen($_SESSION['repita-senha']) == 0){
        ...
        if(!preg_match("/\W/", $_SESSION['senha'])){
        ...
        if(preg_match("/\s/", $_SESSION['senha'])){
            $mensagem_erro[] = "A senha não pode conter nenhum espaço em branco";
        }

        // Verificando se as senhas são iguais.
        if(strcmp($_SESSION['senha'], $_SESSION['repita-senha']) != 0){
        ...
        if(count($mensagem_erro) == 0){
            ...
            if($adicionar_usuario->VerificarEmail($_SESSION['email'])){
                $mensagem_erro[] = "Já existe um usuário com este email";
            }else{
                // Adicionando novo usuário
                if($adicionar_usuario->Cadastrar($_SESSION['nome'], $_SESSION['sobrenome'], $_SESSION['email'], $_SESSION['senha'])){
                    // Removendo sessões
                    unset($_SESSION['nome'], $_SESSION['sobrenome'], $_SESSION['email'], $_SESSION['senha'], $_SESSION['repita-senha']);
...
<body>
    ...
    <!-- PAINEL DE CONTROLE USUÁRIOS - ADICIONAR NOVO USUÁRIO -->
    <div class="container-fluid mt-5 pt-5">
            <div class="row">
                <div class="col-12">
                    <div class="row mb-3 py-3" style="-ms-flex-wrap: nowrap !important; flex-wrap: nowrap !important;">
                        <div class="col-12">
                            <span class="text-preto ml-2 pl-2 border-left-titulo-preto" style="font-size: 15pt;">ADICIONAR NOVO USUÁRIO</span>
                        </div>
                    </div>
                                ...
                                <form method="POST" autocomplete="off">
                                    <div class="form-row mt-3">
                                        <div class="form-group col-6">
                                            <label for="nome-adicionar-usuario" class="text-cinza-2"><i class="fas fa-user pr-2"></i>Nome</label>
                                            <input type="text" class="form-control form-control-alt-2" placeholder="ex: Gleydson" id="nome-adicionar-usuario" name="nome">
                                        </div>
                                        <div class="form-group col-6">
                                            <label for="sobrenome-adicionar-usuario" class="text-cinza-2"><i class="far fa-user pr-2"></i>Sobrenome</label>
                                            <input type="text" class="form-control form-control-alt-2" placeholder="ex: José" id="sobrenome-adicionar-usuario" name="sobrenome">
                                        </div>
                                    </div>
                
                                    <div class="form-group">
                                        <label for="email-adicionar-usuario" class="text-cinza-2"><i class="fas fa-envelope pr-2"></i>E-mail</label>
                                        <input type="text" class="form-control form-control-alt-2" placeholder="ex: satuctgames@hotmail.com" id="email-adicionar-usuario" name="email">
                                    </div>
                
                                    <div class="form-row">
                                        <div class="form-group col-6">
                                            <label for="senha-adicionar-usuario" class="text-cinza-2"><i class="fas fa-lock pr-2"></i>Senha</label>
                                            <input type="password" class="form-control form-control-alt-2" id="senha-adicionar-usuario" name="senha">
                                        </div>
                                        <div class="form-group col-6">
                                            <label for="repita-senha-adicionar-usuario" class="text-cinza-2"><i class="fas fa-key pr-2"></i>Digite a senha novamente</label>
                                            <input type="password" class="form-control form-control-alt-2" id="repita-senha-adicionar-usuario" name="repita-senha">
                                        </div>
                                    </div>
                                    ...
                                    <div class="btn-group w-100 mt-4">
                                        ...
                                        <button type="submit" class="btn btn-cinza-form w-75 py-2 ml-2" name="adicionar-novo-usuario"><i class="fas fa-user-plus mr-2"></i>Adicionar novo usuário</button>
                                    </div>
                                </form>
                            </div>
                        </div>
