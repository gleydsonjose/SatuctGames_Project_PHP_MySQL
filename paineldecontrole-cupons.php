<?php
    ...
    $metodos_cupons = new Dados("nome do banco de dados", "host", "nome de usuario", "senha");

    // Método para buscar cupons existentes no banco de dados.
    $mostrar_cupons = $metodos_cupons->BuscarCupons();

    // Recebendo o id do cupom a partir do botão remover e colocando no método 'RemoverCupom', Depois de remover o cupom, a página será atualizada.
    if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_cupom'])){
        $metodos_cupons->RemoverCupom($_POST['id_cupom']);

        echo "<meta HTTP-EQUIV='refresh' CONTENT='0;URL=paineldecontrole-cupons.php'>";
    }
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <title>Painel de Controle Cupons - SatuctGames</title>
</head>
<body>
    <!-- HEADER -->
                ...
                <nav class="navbar navbar-expand-lg navbar-dark bg-preto menu-loja py-0">
                    ...
                    <div class="collapse navbar-collapse w-100" id="menu-navegacao">
                        <ul class="navbar-nav mt-lg-0 mt-2">
                            ...
                            <li class="nav-item">
                                <a href="paineldecontrole-cupons.php" class="nav-link btn-menu-cinza ativo">Cupons<span class="sr-only">(atual)</span></a>
                            </li>
                        </ul>
    ...
    <!-- PAINEL DE CONTROLE CUPONS -->
    <div class="container-fluid mt-5 pt-5">
        <div class="row">
            <div class="col-12">
                <div class="row mb-5 pt-5 pb-3" style="-ms-flex-wrap: nowrap !important; flex-wrap: nowrap !important;">
                    <div class="col-12">
                        <span class="text-preto ml-2 pl-2 border-left-titulo-preto" style="font-size: 15pt;">CUPONS</span>
                    </div>
                </div>
            </div>  
            ...
            <div class="col-12-lista-painel-de-controle mx-auto">
                ...
                <?php // Mostrando os cupons.
                    foreach($mostrar_cupons as $mc){ ?>
                        <div class="col-1 px-0 d-flex align-items-center justify-content-center border-right-cinza border-bottom-cinza">
                            <h6 class="text-break text-cinza-2 mt-2" style="font-size: 11pt;"><?= $mc['id']?></h6>
                        </div>
    
                        <div class="col-4 col-xl-7 px-0 d-flex align-items-center justify-content-start border-right-cinza border-bottom-cinza">
                            <h6 class="text-break text-cinza-2 mt-2 ml-2" style="font-size: 11pt;"><?= $mc['cupom']?></h6>
                        </div>
    
                        <div class="col-2 col-xl-1 px-0 d-flex justify-content-center align-items-center border-right-cinza border-bottom-cinza">
                            <h6 class="text-break text-cinza-2 mt-2" style="font-size: 11pt;"><?= $mc['desconto']?>%</h6>
                        </div>

                        <div class="col-3 col-xl-2 px-0 d-flex justify-content-center align-items-center border-right-cinza border-bottom-cinza">
                            <h6 class="text-break text-cinza-2 mt-2" style="font-size: 11pt;"><?= (new DateTime($mc['datadecriacao']))->format('d/m/Y H:i:s') ?></h6>
                        </div>

                        <div class="col-2 col-xl-1 px-0 d-flex flex-column justify-content-center align-items-center border-bottom-cinza">
                            <button type="button" class="btn btn-danger w-100 h-100 d-flex justify-content-center align-items-center" data-toggle="modal" data-target="#remover-cupom-modal<?= $mc['id'] ?>"><i class="fas fa-trash pr-2"></i>Remover</button>
                        </div>

                        <!-- REMOVER CUPOM -->
                        <div class='modal fade' id='remover-cupom-modal<?= $mc['id'] ?>' tabindex='-1' role='dialog' aria-label='remover-cupom-modal' aria-hidden='true'>
                                ...
                                <div class='modal-content'>
                                    <div class='modal-body'>
                                        ...
                                        <div class='row d-flex justify-content-center mt-4 mb-3'>
                                            <p class='text-cinza-2'>Você tem certeza que quer remover o cupom <?= $mc['cupom']?>?</p>
                                        </div>

                                        <form method="POST" class='row d-flex justify-content-center mb-2'>
                                            <button type="submit" class='btn btn-success py-1 mr-2' style='width: 60px;' name="id_cupom" <?php echo "value='$mc[id]'"; ?>>Sim</button>
                ...
                <div class="row my-5">
                    <div class="col-12 d-flex justify-content-center">
                        <a href="paineldecontrole-cupons-criar-cupom.php" class="btn btn-cinza-form py-2 px-3"><i class="fas fa-plus mr-2"></i>Criar novo cupom</a>
                    </div>
                </div>
            </div>
        </div>
